...
**Blocked on** (Epic Known Conflict 1): landed `CellContents` carries only
`block_type_id` + `material_id` — it has **no furniture concept**. A built
furniture cell reads non-empty (solid) and would count as floor/wall/roof and
break clearance, directly contradicting Rule 1's transparency clause. The
furniture-transparency AC below requires an injected furniture-occupancy source
from `building-028`; do not synthesize one from `CellContents`.
...
- [ ] For this analysis, furniture occupancy is **transparent**: a cell occupied by furniture evaluates as if empty (furniture never counts as floor, wall, or roof), so a furniture item's own cell has a well-defined candidate status. [TR-022]
...
- Standability is **not** re-derived here: call the injected walkability provider's `is_standable(cell)`. Its landed implementation already checks solid-below + `VILLAGER_CLEARANCE` passability and treats out-of-bounds as blocking.
...
- **Furniture transparency**: Given a furniture-occupied cell inside a roofed, floored space, When analyzed, Then the cell's candidate status is identical to the same cell with the furniture absent; and given furniture placed where a wall/floor/roof would be needed, Then it does NOT satisfy that structural role.
...
- Depends on: 001 (config + walkability provider wiring). **External**: `building-028` furniture placement for the furniture-transparency AC (Epic Known Conflict 1).
- Unlocks: 003, 010
