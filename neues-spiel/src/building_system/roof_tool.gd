...
## `setup()`/`is_set_up()` gate exists here, mirroring [FloorTool]'s identical
## no-setup precedent. [method resolve_cell_set] is callable immediately
## after construction (formation defaults to [constant Formation.FLAT]).
class_name RoofTool
extends Node
