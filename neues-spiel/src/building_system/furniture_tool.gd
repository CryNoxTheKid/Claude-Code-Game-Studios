...
##
## **Single-cell place, always the ATTACH cell** (mirrors [BlockTool]'s own
## established precedent exactly): [method resolve_cell_set] ignores both its
## own `is_drag` parameter and `release_cell` entirely -- only the press
## matters. Multi-cell footprint placement (MVP: `bed` = 1x2) is Story
## building-016's extension of this tool, not this story's.
##
...
## doc comment for why only the press matters for this single-cell tool.
func resolve_cell_set(_is_drag: bool, press_cell: Vector3i, _release_cell: Vector3i) -> Array[Vector3i]:
	return FurnitureTool.furniture_cell_set(press_cell)


## GDD Core Rule 8 (pure, stateless -- exercisable directly with arbitrary
## values, mirroring [method BlockTool.block_cell_set]'s established
## testable-pure-function shape): a furniture-tool commit is always exactly
## the one picked ATTACH cell (this story's single-cell scope; Story
## building-016 replaces this with the item's real multi-cell footprint).
static func furniture_cell_set(press_cell: Vector3i) -> Array[Vector3i]:
	return [press_cell]
