...
- **`current_cell` changes at EXACTLY two sanctioned points, both tick-boundary discrete**: (a) tick-boundary travel arrival, and (b) an unstuck-watchdog rescue. The watchdog teleport is deterministic (fires at a deterministic stuck-tick threshold; target chosen by expanding-ring BFS with the F2 lexicographic `y,x,z` tie-break), sets `current_cell` atomically and **snaps `_visual_position` to match** (no lerp for a rescue) — source: ADR-0009 (slice propagation) + villager-ai Rule 15/F5
